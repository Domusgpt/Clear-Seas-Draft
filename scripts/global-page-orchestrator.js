...
function shouldDispatchMotionEvent(previous, next) {
  ...
  return Object.keys(thresholds).some((key) => {
    ...
    const slots = Math.max(previousGroups.length, nextGroups.length, 1);
    for (let index = 0; index < Math.min(slots, 3); index += 1) {
      const prev = previousGroups[index] || {};
      const curr = nextGroups[index] || {};
      if ((prev.id || 0) !== (curr.id || 0)) {
        return true;
      }
      const synergyDelta = Math.abs((curr.synergy || 0) - (prev.synergy || 0));
      const focusDelta = Math.abs((curr.focus || 0) - (prev.focus || 0));
      const focusXDelta = Math.abs((curr.focusX || 0) - (prev.focusX || 0));
      const focusYDelta = Math.abs((curr.focusY || 0) - (prev.focusY || 0));
      if (synergyDelta > 0.004 || focusDelta > 0.004 || focusXDelta > 0.003 || focusYDelta > 0.003) {
        return true;
      }
    }
    return false;
...
function maybeDispatchGlobalMotionEvent() {
  const detail = {
    ...
    groups: Array.isArray(sharedMotion.groups)
      ? sharedMotion.groups.map((group) => ({ ...group }))
      : [],
...
function attachToGroup(state) {
  ...
  if (!group.dataset.globalGroupId) {
    group.dataset.globalGroupId = String(groupState.id);
  }
...
function detachFromGroup(state) {
  ...
  if (groupState.cards.size === 0) {
    ...
    element.removeAttribute('data-global-group-id');
...
function step() {
  ...
  const rankedGroups = Array.from(groupStates.values())
    .filter((groupState) => groupState.cards && groupState.cards.size > 0)
    .sort((a, b) => b.synergy.current - a.synergy.current);
  topGroupData = rankedGroups.slice(0, 3).map((groupState, index) => ({
    id: groupState.id || index + 1,
    synergy: groupState.synergy.current,
    focus: groupState.focus.current,
    focusX: groupState.pointer.currentX,
    focusY: groupState.pointer.currentY
  }));
  ...
  root.style.setProperty('--global-active-group-count', String(topGroupData.length));
  const maxTrackedGroups = 3;
  for (let i = 0; i < maxTrackedGroups; i += 1) {
    const slot = i + 1;
    const data = topGroupData[i];
    const synergy = data ? data.synergy : 0;
    const focus = data ? data.focus : 0;
    const focusX = data ? data.focusX : 0.5;
    const focusY = data ? data.focusY : 0.5;
    const idValue = data && data.id ? String(data.id) : '0';
    root.style.setProperty(`--global-group-${slot}-synergy`, synergy.toFixed(4));
    root.style.setProperty(`--global-group-${slot}-focus`, focus.toFixed(4));
    root.style.setProperty(`--global-group-${slot}-focus-x`, focusX.toFixed(4));
    root.style.setProperty(`--global-group-${slot}-focus-y`, focusY.toFixed(4));
    root.style.setProperty(`--global-group-${slot}-id`, idValue);
  }
  const compositeGroupSynergy = topGroupData.reduce((total, group, index) => (
    total + group.synergy * (1 - index * 0.25)
  ), 0);
  root.style.setProperty('--global-top-groups-synergy', compositeGroupSynergy.toFixed(4));
  ...
  sharedMotion.groups = topGroupData.map((group, index) => ({
    id: group.id,
    rank: index + 1,
