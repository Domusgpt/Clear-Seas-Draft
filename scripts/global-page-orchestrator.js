...
function computeVisibilityFactor(state) {
  if (!state) {
    return 0;
  }
  if (typeof state.visibilityRatio === 'number') {
    return Math.max(0, Math.min(1, state.visibilityRatio));
  }
  return state.isVisible ? 1 : 0;
}

function shouldActivateBrandVideo(state) {
  if (!state || !state.element) {
    return false;
  }
  const visibility = computeVisibilityFactor(state);
  if (visibility < 0.35) {
    return false;
  }
  const focusCurrent = state.focus?.current || 0;
  const focusTarget = state.focus?.target || 0;
  const support = Math.max(0, state.support?.current || 0);
  const focus = Math.max(focusCurrent, focusTarget);
  const threshold = 0.35 + support * 0.25;
  return focus >= threshold;
}

function shouldSustainBrandVideo(state) {
  if (!state || !state.element) {
    return false;
  }
  const visibility = computeVisibilityFactor(state);
  ...
  const focusCurrent = state.focus?.current || 0;
  return focusCurrent >= 0.22;
}

function applyOverlayImage(state, overlay, imageSource) {
  if (!overlay) {
    return;
  }
  if (state.brandVideo) {
  ...
  overlay.dataset.brandVideoActive = 'false';
  overlay.dataset.brandVideoPending = 'false';
  if (imageSource) {
...
function applyOverlayVideo(state, overlay, videoSource) {
  if (!overlay || !videoSource) {
    return;
  }
  overlay.dataset.brandVideoActive = 'true';
  overlay.dataset.brandVideoPending = 'false';
  overlay.style.backgroundImage = 'none';

  const needsNewVideo = !state.brandVideo || state.currentVideoSrc !== videoSource || !overlay.contains(state.brandVideo);

  if (needsNewVideo) {
    if (state.brandVideo && overlay.contains(state.brandVideo)) {
      try {
        state.brandVideo.pause();
      } catch (error) {
        // ignore pause failures
    ...
    video.addEventListener('canplay', () => {
      if (shouldSustainBrandVideo(state)) {
        video.play().catch(() => {});
      }
    });
    overlay.appendChild(video);
    state.brandVideo = video;
    state.currentVideoSrc = videoSource;
  }
...
function ensureBrandLayer(state) {
  ...
  if (preferVideo) {
    videoSource = state.pendingVideoSrc || brandOverrideApi.pickAsset(state.overrides, 'videos', cycleIndex, (index) => pickBrandVideo(index));
  ...
  const canActivateVideo = Boolean(videoSource) && shouldActivateBrandVideo(state);
  if (canActivateVideo && videoSource) {
    state.pendingVideoSrc = null;
    applyOverlayVideo(state, overlay, videoSource);
  } else {
    state.pendingVideoSrc = videoSource || null;
    const fallbackImage = imageSource || state.lastImageSource || null;
    applyOverlayImage(state, overlay, fallbackImage);
    overlay.dataset.brandVideoPending = state.pendingVideoSrc ? 'true' : 'false';
  }
...
function step() {
  ...
  cardStates.forEach((state, element) => {
    ...
    if (state.overlay) {
      if (state.pendingVideoSrc && shouldActivateBrandVideo(state)) {
        state.overlay = ensureBrandLayer(state);
        continueAnimation = true;
      } else if (state.brandVideo && !shouldSustainBrandVideo(state)) {
        const pendingSource = state.currentVideoSrc || (state.brandVideo?.currentSrc || state.brandVideo?.src) || null;
        state.pendingVideoSrc = pendingSource;
        applyOverlayImage(state, state.overlay, state.lastImageSource || null);
        state.overlay.dataset.brandVideoPending = state.pendingVideoSrc ? 'true' : 'false';
        continueAnimation = true;
      }
    }
  ...
  const tiltBase = 0.58 + globalState.synergy.current * 0.22;
  const scrollInfluence = globalState.scroll.current * 0.12;
  globalState.tilt.targetX = (focusX - 0.5) * tiltBase + scrollInfluence;
  globalState.tilt.targetY = (0.5 - focusY) * (tiltBase * 0.9) - scrollInfluence * 0.45;

  const synergyWeight = globalState.synergy.current * 0.4;
  const momentumWeight = Math.min(0.45, Math.abs(globalState.scroll.current) * 0.55);
  globalState.bend.target = Math.min(1, focusAmount * 0.5 + synergyWeight + momentumWeight);
  globalState.warp.target = Math.max(-1, Math.min(1, (globalState.tilt.targetX - globalState.tilt.targetY) * 0.35));
  ...
  globalState.tilt.currentX += (globalState.tilt.targetX - globalState.tilt.currentX) * 0.12;
  globalState.tilt.currentY += (globalState.tilt.targetY - globalState.tilt.currentY) * 0.12;
  ...
  const tiltStrength = Math.min(0.72, Math.sqrt((globalState.tilt.currentX ** 2) + (globalState.tilt.currentY ** 2)) * 0.85 + globalState.bend.current * 0.32);
...
function isOrchestratorDisabled() {
  const root = document.documentElement;
  const body = document.body;
  if (!root) {
    return false;
  }
  const datasetDisabled = root.dataset.disableGlobalOrchestrator === 'true' || (body && body.dataset.disableGlobalOrchestrator === 'true');
  const classDisabled = root.classList.contains('disable-global-orchestrator') || (body && body.classList.contains('disable-global-orchestrator'));
  return datasetDisabled || classDisabled;
}
...
const orchestratorDisabled = isOrchestratorDisabled();
if (orchestratorDisabled) {
  window.__CLEAR_SEAS_GLOBAL_ORCHESTRATOR_DISABLED = true;
