...
const intersectionThresholds = Array.from({ length: 11 }, (_, index) => index / 10);
...
function ensureBrandLayer(state) {
  ...
  overlay.style.setProperty('--overlay-shift-x', '0px');
  overlay.style.setProperty('--overlay-shift-y', '0px');
...
function handleVisibilityEntries(entries) {
  entries.forEach((entry) => {
    const state = cardStates.get(entry.target);
    if (!state) {
      if (viewportObserver) {
    ...
    state.visibility.target = ratio;

    if (ratio < 0.05 && state !== activeCardState) {
      state.focus.target *= 0.6;
      state.support.target *= 0.7;
...
function ensureViewportObserver() {
  if (!viewportObserver) {
    viewportObserver = new IntersectionObserver(handleVisibilityEntries, {
      threshold: intersectionThresholds,
      rootMargin: '18% 0px 18% 0px'
    });
    cardStates.forEach((_, element) => viewportObserver.observe(element));
  }
...
function createState(element, index) {
  ...
  element.style.setProperty('--card-visibility', '0');
  element.style.setProperty('--card-ambient-focus', '0');
  element.style.setProperty('--card-cohesion', '0');
  return state;
}
...
function updateSupportTargets(activeState) {
  ...
  groupStates.forEach((groupState, groupElement) => {
    if (!activeGroup) {
      groupState.synergy.target = Math.max(groupState.synergy.target * 0.75, 0.08);
  ...
  if (activeState) {
    const visibility = activeState.visibility
      ? (activeState.visibility.current || activeState.visibility.target || 0)
      : 0;
    const focusEstimate = Math.max(activeState.focus.current || 0, activeState.focus.target || 0);
    const synergyTarget = 0.35 + visibility * 0.45 + focusEstimate * 0.35;
    globalState.synergy.target = Math.min(1, Math.max(globalState.synergy.current, synergyTarget));
  } else {
    globalState.synergy.target = Math.max(0, globalState.synergy.current * 0.6);
  }
  requestTick();
}
...
function step() {
  ...
  cardStates.forEach((state, element) => {
    ...
    state.visibility.current += (state.visibility.target - state.visibility.current) * 0.18;

    const groupState = state.group ? groupStates.get(state.group) : null;
    const groupFocus = groupState ? Math.max(0, Math.min(1, groupState.focus.current)) : 0;
    const groupSynergy = groupState ? Math.max(0, Math.min(1, groupState.synergy.current)) : 0;

    if (groupState && state !== activeCardState) {
      const followBlend = 0.08 + groupFocus * 0.12;
      state.pointer.smoothX += (groupState.pointer.currentX - state.pointer.smoothX) * followBlend;
      state.pointer.smoothY += (groupState.pointer.currentY - state.pointer.smoothY) * followBlend;
      const twistTarget = (state.pointer.smoothX - 0.5) * groupSynergy * 26;
      state.twist.current += (twistTarget - state.twist.current) * 0.08;
    }

    const ambientFocus = state === activeCardState ? 0 : groupFocus * 0.45 * Math.max(0.2, state.visibility.current);
    const focusStrength = Math.max(0, Math.min(1.2, state.focus.current + ambientFocus));
    let supportStrength = Math.max(-1, Math.min(1, state.support.current));
    if (state !== activeCardState) {
      supportStrength = Math.max(-1, Math.min(1, supportStrength + groupSynergy * 0.08));
    }
    ...
    element.style.setProperty('--card-visibility', state.visibility.current.toFixed(4));
    element.style.setProperty('--card-ambient-focus', ambientFocus.toFixed(4));
    element.style.setProperty('--card-cohesion', groupSynergy.toFixed(4));

    if (state.overlay) {
      const overlayShiftX = (state.pointer.smoothX - 0.5) * (48 + groupSynergy * 32) + globalState.scroll.current * 18;
      const overlayShiftY = (state.pointer.smoothY - 0.5) * -42 + groupSynergy * -6;
      state.overlay.style.setProperty('--overlay-shift-x', `${overlayShiftX.toFixed(2)}px`);
      state.overlay.style.setProperty('--overlay-shift-y', `${overlayShiftY.toFixed(2)}px`);
      const overlayOpacity = Math.min(0.9, Math.max(0.12, 0.24 + focusStrength * 0.55 + groupSynergy * 0.25));
      state.overlay.style.setProperty('--overlay-opacity', overlayOpacity.toFixed(3));
    }
    ...
    if (Math.abs(state.pointer.targetX - state.pointer.smoothX) > 0.001 ||
        ...
        Math.abs(state.visibility.target - state.visibility.current) > 0.001 ||
