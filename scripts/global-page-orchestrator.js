...
// Determine the active collection so every page inherits the right color story and animation weights.
function computePageContext() {
  const path = (window.location.pathname || '').toLowerCase();
  const hasClearSeas = Boolean(document.querySelector('link[href*="clear-seas-ai.css"], script[src*="clear-seas-ai.js"]'));
  const hasConsolidated = Boolean(document.querySelector('link[href*="consolidated-styles.css"]'));
  const hasAvantGarde = Boolean(document.querySelector('link[href*="avant-garde.css"]'));
  const isUltimate = /ultimate-clear-seas-holistic-system/.test(path);
  const isOrthogonal = /orthogonal-depth-progression/.test(path);
  const isIndexCollection = /(?:^|\/)(?:index|[1-6]-index[^/]*?)\.html$/.test(path);
  const isRoot = path === '/' || path === '';
  const prMatch = path.match(/pr-(\d+)/);
  const prNumber = prMatch ? Number.parseInt(prMatch[1], 10) : null;

  if (isUltimate) {
    return { id: 'ultimate-showcase', cluster: 'ultimate', tier: 'pinnacle', prNumber };
  }
  if (hasClearSeas) {
    const tier = prNumber ? (prNumber >= 22 ? 'recent' : prNumber >= 15 ? 'mid' : 'early') : 'recent';
    return { id: prNumber ? `immersive-pr-${prNumber}` : 'immersive-suite', cluster: 'immersive', tier, prNumber };
  }
  if (hasConsolidated || isIndexCollection || isRoot) {
    return { id: (isIndexCollection || isRoot) ? 'core-index' : 'core-shared-styles', cluster: 'core', tier: 'foundation', prNumber };
  }
  if (hasAvantGarde || isOrthogonal) {
    return { id: isOrthogonal ? 'orthogonal-lab' : 'concept-lab', cluster: 'labs', tier: 'exploration', prNumber };
  }
  return { id: prNumber ? `experimental-pr-${prNumber}` : 'general', cluster: 'general', tier: prNumber ? 'exploration' : 'general', prNumber };
}

function applyPageContext() {
  const context = computePageContext();
  const root = document.documentElement;
  const body = document.body;
  const profile = clusterProfiles[context.cluster] || clusterProfiles.general;
  ...
  const tierMultiplier = tierAdjustments[context.tier] || 1;
  const overlayStrength = (profile.overlay * tierMultiplier).toFixed(3);
  const sparkIntensity = (profile.spark * tierMultiplier).toFixed(3);

  if (root) {
    root.dataset.pageCluster = context.cluster;
    root.dataset.pageProfile = context.id;
    root.dataset.pageTier = context.tier;
    if (context.prNumber) {
      root.dataset.pagePrNumber = String(context.prNumber);
    } else {
      root.removeAttribute('data-page-pr-number');
    }
    root.style.setProperty('--cluster-primary-rgb', profile.primary);
    root.style.setProperty('--cluster-secondary-rgb', profile.secondary);
    root.style.setProperty('--cluster-tertiary-rgb', profile.tertiary);
    root.style.setProperty('--cluster-overlay-strength', overlayStrength);
    root.style.setProperty('--cluster-spark-intensity', sparkIntensity);
  }

  if (body) {
    body.dataset.pageCluster = context.cluster;
    body.dataset.pageProfile = context.id;
    body.dataset.pageTier = context.tier;
    if (context.prNumber) {
      body.dataset.pagePrNumber = String(context.prNumber);
    } else {
      body.removeAttribute('data-page-pr-number');
    }
    const collection = context.cluster === 'immersive'
      ? 'immersive-ai-command'
      : context.cluster === 'core'
        ? 'core-foundation'
        : context.cluster === 'labs'
          ? 'concept-labs'
          : context.cluster === 'ultimate'
            ? 'ultimate-holistic'
            : 'general';
    body.dataset.pageCollection = collection;
    if (root) {
      root.dataset.pageCollection = collection;
    }
  }
}
...
function detachFromGroup(state) {
  ...
  applyCompanionState(state, state.isVisible ? 'ambient' : 'dormant', state.isVisible ? 0.12 : -0.22);
}
...
function createState(element, index) {
  ...
  applyCompanionState(state, 'ambient', 0.12);
...
function applyCompanionState(state, role, weight) {
  const clampedWeight = Number.isFinite(weight) ? weight : 0;
  state.element.dataset.companionState = role;
  state.element.style.setProperty('--card-companion-weight', clampedWeight.toFixed(3));
}
...
function updateSupportTargets(activeState) {
  ...
  if (!activeState) {
    cardStates.forEach((state) => {
      if (state.isVisible) {
        ...
        applyCompanionState(state, 'ambient', 0.16);
      } else {
        state.support.target = 0;
        state.element.dataset.supportRole = 'neutral';
        state.element.dataset.supportDistance = 'far';
        applyCompanionState(state, 'dormant', -0.22);
      }
    });
  ...
  cardStates.forEach((state) => {
    if (state === activeState) {
      ...
      applyCompanionState(state, 'primary', 0.92);
      return;
    }
    ...
    if (activeGroup && state.group === activeGroup && activeIndex !== -1) {
      ...
      let intensity = -0.02;
      let companionWeight = state.isVisible ? 0.1 : -0.18;
      let companionRole = state.isVisible ? 'ambient' : 'dormant';
      if (state.isVisible) {
        if (distance === 1) {
          intensity = 0.22;
          companionWeight = 0.58;
          companionRole = 'adjacent';
        } else if (distance === 2) {
          intensity = 0.1;
          companionWeight = 0.34;
          companionRole = 'halo';
        } else if (distance === 3) {
          companionWeight = 0.18;
        }
      }
      state.support.target = intensity;
      state.element.dataset.supportRole = 'supporting';
      applyCompanionState(state, companionRole, companionWeight);
    } else {
      state.element.dataset.supportDistance = 'far';
      state.support.target = state.isVisible ? -0.06 : -0.12;
      state.element.dataset.supportRole = 'ambient';
      applyCompanionState(state, state.isVisible ? 'ambient' : 'dormant', state.isVisible ? 0.12 : -0.24);
    }
  });
