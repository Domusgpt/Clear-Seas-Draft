...
class VIB34DTiltVisualizer {
    ...
    getTargetsForState(state) {
        const base = this.presets;
        switch (state) {
            case 'approaching':
                return {
                    speed: base.baseSpeed * 1.1,
                    glitch: base.baseGlitch * 0.8,
                    density: base.baseDensity * 0.85,
                    moire: base.baseMoire * 0.7,
                    energy: 0.65
                };
            case 'focused':
                return {
                    speed: base.baseSpeed * 1.75,
                    glitch: base.baseGlitch * 1.4 + 0.2,
                    density: base.baseDensity * 0.6,
                    moire: base.baseMoire * 1.3 + 0.1,
                    energy: 1.05
                };
            case 'exiting':
                return {
                    speed: base.baseSpeed * 1.35,
                    glitch: base.baseGlitch * 1.1,
                    density: base.baseDensity * 0.75,
                    moire: base.baseMoire * 0.9,
                    energy: 0.5
                };
            case 'destroyed':
                return {
                    speed: base.baseSpeed * 2.1,
                    glitch: base.baseGlitch * 1.9 + 0.4,
                    density: base.baseDensity * 0.35,
                    moire: base.baseMoire * 1.6 + 0.2,
                    energy: 1.2
                };
            case 'far-depth':
            default:
                return {
                    speed: base.baseSpeed * 0.6,
                    glitch: base.baseGlitch * 0.45,
                    density: base.baseDensity * 1.25,
                    moire: base.baseMoire * 0.4,
                    energy: 0.2
                };
        }
    }

    realignToState(options = {}) {
        const targets = this.getTargetsForState(this.cardState || 'far-depth');
        Object.assign(this.targetState, targets);
        if (this.activeTrait) {
            this.applyInheritedTrait(this.activeTrait, { ...options, silent: options.silent !== false, immediate: options.immediate || false });
        }
    }
    ...
    setCardState(state, options = {}) {
        ...
        const targets = this.getTargetsForState(state);
        Object.assign(this.targetState, targets);
    ...
    applyRelationalResponse(type, payload = {}, context = 'focused') {
        const intensity = typeof payload.intensity === 'number' ? payload.intensity : 0;
        const baseTargets = this.getTargetsForState(this.cardState || 'far-depth');
        const focusFactor = context === 'focused' ? 1 : -0.45;

        switch (type) {
            case 'pointer-move': {
                this.targetState.glitch = this.clamp(baseTargets.glitch + intensity * 0.75 * focusFactor, 0, 3);
                this.targetState.moire = this.clamp(baseTargets.moire + intensity * 0.52 * focusFactor, 0, 3);
                this.targetState.density = this.clamp(baseTargets.density - intensity * 0.38 * focusFactor, 0.2, 1.8);
                this.targetState.speed = baseTargets.speed + intensity * (context === 'focused' ? 0.55 : 0.25);
                this.targetState.energy = this.clamp(baseTargets.energy + intensity * (context === 'focused' ? 0.68 : 0.28), 0, 3);
                break;
            }
            case 'pointer-press': {
                this.targetState.glitch = this.clamp(baseTargets.glitch + intensity * (context === 'focused' ? 1.05 : 0.4), 0, 3);
                this.targetState.moire = this.clamp(baseTargets.moire + intensity * (context === 'focused' ? 0.6 : 0.25), 0, 3);
                this.targetState.energy = this.clamp(baseTargets.energy + intensity * (context === 'focused' ? 0.9 : 0.35), 0, 3);
                if (context === 'focused') {
                    this.traitFlourish = {
                        active: true,
                        start: performance.now(),
                        duration: 900,
                        trait: this.activeTrait
                    };
                }
                break;
            }
            case 'pointer-release': {
                this.targetState.glitch = this.clamp(this.targetState.glitch * 0.92, 0, 3);
                this.targetState.energy = this.clamp(this.targetState.energy * 0.9 + baseTargets.energy * 0.1, 0, 3);
                this.targetState.moire = this.clamp(this.targetState.moire * 0.94, 0, 3);
                break;
            }
            case 'pointer-neutral': {
                this.realignToState({ silent: true });
                break;
            }
        }
    }
