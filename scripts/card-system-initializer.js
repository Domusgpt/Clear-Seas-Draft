...
class CardSystemController {
  ...
  applyCardSynergy(cardData, tiltX, tiltY) {
    ...
    const parallaxX = 0;
    const parallaxY = fusedTiltPitch * -parallaxStrength;
    ...
    cardData.element.style.setProperty('--focus-tilt-x-deg', `${(-fusedTiltPitch * 14).toFixed(2)}deg`);
    cardData.element.style.setProperty('--focus-tilt-y-deg', '0deg');
    cardData.element.style.setProperty('--focus-parallax-x', '0px');
    ...
    const sharedTiltTarget = this.scrollTilt + fusedTiltPitch * 0.45 + sharedTiltY * 0.35;
    ...
    const pointerMagnitude = Math.min(1.25, Math.abs(fusedTiltPitch));
    ...
    const bendTiltX = -fusedTiltPitch * 12 * bendIntensity;
    const bendTiltY = 0;
    const bendSkewX = -(sharedTiltY * 0.2) * 6 * bendIntensity;
    const bendSkewY = (fusedTiltPitch + sharedTiltY * 0.2) * 5 * bendIntensity;
