...
class VIB34DContainedVisualizer {
  ...
  setGlobalMotion(motion = {}) {
    ...
    if (this.card) {
      ...
      this.card.style.setProperty('--shared-tilt-x', '0.0000');
  ...
  updateMouseBasedParameters() {
    ...
    const globalVerticalTilt = motion.tiltY || 0;
    ...
    const horizontalPointer = this.mouseState.x - 0.5;
    const verticalPointer = 0.5 - this.mouseState.y;

    this.dynamicParams.rot4dXW = horizontalPointer * Math.PI * 0.6 + globalVerticalTilt * Math.PI * 0.55;
    this.dynamicParams.rot4dYW = verticalPointer * Math.PI * 0.85 + globalVerticalTilt * Math.PI * 0.35;
    ...
    const morphModulation = this.parameters.morphFactor + (verticalPointer * -0.8) + globalSynergy * 0.35;
    const chaosModulation = this.parameters.chaos + (horizontalPointer * 0.3) + globalWarp * 0.2;
    ...
    const tiltXDeg = verticalPointer * 18 - globalVerticalTilt * 22;
    const tiltYDeg = 0;
    ...
    this.card.style.setProperty('--tilt-y-value', '0');
